...
def format_value(value):
    if isinstance(value, int) and 1000000000 <= value <= 9999999999:
        try:
...
def format_dict_to_text(d, indent=0, header=None):
    lines = []
    ...
    for key, value in d.items():
        ...
        if isinstance(value, dict):
            lines.extend(format_dict_to_text(value, indent + 2, formatted_key))
        else:
